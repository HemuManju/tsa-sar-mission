...
def build_chat(ui_batch):
    ...
    hist_doc.set_style(
        0, 0, dict(color=COLOR_TEXT, font_name=DEFAULT_FONT, font_size=11)
    )
    hist_layout = IncrementalTextLayout(
        hist_doc,
        width=SIDEBAR_W - 30,
    ...
    input_doc.set_style(
        0, 0, dict(color=(240, 240, 255, 255), font_name=DEFAULT_FONT, font_size=12)
    )
    input_layout = IncrementalTextLayout(
        input_doc, width=SIDEBAR_W - 40, height=26, multiline=False, batch=ui_batch
    )
    input_layout.x = PLAY_W_PX + 20
    input_layout.y = 40

    input_box = shapes.BorderedRectangle(
        PLAY_W_PX + 15,
        35,
        SIDEBAR_W - 30,
        34,
...
def chatui():
    return {"build_chat": build_chat, "append_line": append_line}
