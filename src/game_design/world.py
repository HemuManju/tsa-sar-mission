...
from config import (
    ...
    PURPLE_SECTORS_X,
    PURPLE_SECTORS_Y,
    RED_DIFFICULTY_CORRIDOR_BONUS,
    RED_DIFFICULTY_DEADEND_BONUS,
    RED_DIFFICULTY_DIST_WEIGHT,
    RED_DIFFICULTY_NEARWALL_BONUS,
    RED_FAR_QUANTILE,
    RED_SECTORS_X,
    RED_SECTORS_Y,
...
def victim_color(k):
    return (
        COLOR_YELLOW
        if k == "yellow"
        else (COLOR_PURPLE if k == "purple" else COLOR_RED)
    )
...
def _border_cells():
    return (
        {(x, 0) for x in range(PLAY_W)}
        | {(x, PLAY_H - 1) for x in range(PLAY_W)}
        | {(0, y) for y in range(PLAY_H)}
        | {(PLAY_W - 1, y) for y in range(PLAY_H)}
    )
...
def _comps(wset):
    ...
    for c in wset:
        ...
        while st:
            x, y = st.pop()
            if (x, y) in vis or (x, y) not in wset:
                continue
            vis.add((x, y))
            comp.add((x, y))
            for nx, ny in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
                if (nx, ny) in wset and (nx, ny) not in vis:
                    st.append((nx, ny))
        comps.append(comp)
    return comps
...
def generate_walls(difficulty: str):
    ...
    while placed < segs and attempts > 0 and len(walls) < max_walls:
        ...
        if (sx, sy) in reserved or (sx, sy) == START:
            continue
        ...
        for _ in range(length):  # ensures only valid, non-overlapping wall segments
            ...
            x += dx
            y += dy
        if not ok or not prop or any(p in walls for p in _buf(prop)):
            continue  # buffer overlap
                ...
                for x, y in comp:
                    for nx, ny in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
                        if _in(nx, ny):
                            rim.add((nx, ny))
                elig = [
                    c for c in rim if c not in walls and c not in (reserved - comp_buf)
                ]
                if not elig:
                    break
...
# -------- victim placement --------
def place_victims(
    distmap, start, all_passable=None
):  # distmap: how far every cell is from start
    """
    ...
    pool = [
        p for p in distmap.keys() if p != start
    ]  # pool = the initial list of candidate cells where victims could be placed.
    ...
    cutoff = (
        scored_linear[q_idx][1] if 0 <= q_idx < len(scored_linear) else 0
    )  # cutoff distance
    ...
    passable = (
        set(all_passable)
        if all_passable is not None
        else set(p for p, _ in scored_linear)
    )
    dirs4 = ((1, 0), (-1, 0), (0, 1), (0, -1))

    def degree(p):
        x, y = p
        return sum(((x + dx, y + dy) in passable) for dx, dy in dirs4)

    def is_corridor(p):
        x, y = p
        neigh = [(x + dx, y + dy) for dx, dy in dirs4 if (x + dx, y + dy) in passable]
        if len(neigh) != 2:
            return False
        (x1, y1), (x2, y2) = neigh
    ...
    def blocked_sides(p):
        x, y = p
        return 4 - sum(((x + dx, y + dy) in passable) for dx, dy in dirs4)
    ...
    def hardness_score(p):
        ...
        if degree(p) == 1:
            s += RED_DIFFICULTY_DEADEND_BONUS
        if is_corridor(p):
            s += RED_DIFFICULTY_CORRIDOR_BONUS
        if blocked_sides(p) >= 2:
            s += RED_DIFFICULTY_NEARWALL_BONUS
        return s
    ...
    while len(reds) < NUM_RED and sep >= 2:
        placed_any = False
        for i in order:
            if len(reds) >= NUM_RED or quotas[i] <= 0:
                continue
            lst = buckets[i]
            j = 0
            while j < len(lst):
    ...
    if len(reds) < NUM_RED:
        ...
        for p in remaining:
            if len(reds) >= NUM_RED:
                break
            if all(cheby(p, q) >= 2 for q in reds):
                reds.append(p)
    ...
    victims.update(
        distribute_victims(
            rest, NUM_PURPLE, PURPLE_SECTORS_X, PURPLE_SECTORS_Y, "purple"
        )
    )
    ...
    victims.update(
        distribute_victims(
            rest, NUM_YELLOW, YELLOW_SECTORS_X, YELLOW_SECTORS_Y, "yellow"
        )
    )
