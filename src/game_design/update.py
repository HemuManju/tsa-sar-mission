...
def _handle_rescue(game, pos):
    ...
    if hasattr(game, "rescue_positions") and game.rescue_positions:
        if pos in game.rescue_positions:
            if (
                hasattr(game, "carried")
                and isinstance(game.carried, list)
                and game.carried
            ):
                ...
                append_line(
                    game.chat, f"[SYSTEM] Dropped {saved_n} victim(s) at rescue point."
                )
            ...
            if not game.victims and not getattr(game, "carried", []):
                if not game.game_over:
                    ...
                    append_line(
                        game.chat, "[SYSTEM] Mission complete! All victims handled."
                    )
            return

    # Legacy single-rescue behavior (kept for compatibility)
    if (
        getattr(game, "rescue_pos", None)
        and pos == game.rescue_pos
        and not game.rescue_reached
    ):
