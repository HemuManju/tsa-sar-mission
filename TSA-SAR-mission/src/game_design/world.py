...
except: MULTI_WALL_PCT = 0.30
try:    from config import MULTI_WALL_LAYERS
except: MULTI_WALL_LAYERS = (1,3)
try:    from config import MULTI_WALL_GROW_PCT
except: MULTI_WALL_GROW_PCT = 0.35      # fraction of eligible rim cells per layer
try:    from config import MIN_PASSABLE_RATIO
except: MIN_PASSABLE_RATIO = 0.45        # keep ≥45% of cells empty (playable)

# Sanitize types so bad config won't crash
def _f01(v, d):  # clamp to [0,1]
    try:
...
MULTI_WALL_PCT    = _f01(MULTI_WALL_PCT, 0.30)
MULTI_WALL_GROW_PCT = _f01(MULTI_WALL_GROW_PCT, 0.35)
MULTI_WALL_LAYERS = _pair(MULTI_WALL_LAYERS, (1,3))
MIN_PASSABLE_RATIO = _f01(MIN_PASSABLE_RATIO, 0.45)
...
def generate_walls(diff: str):
    """Borders + random segments with clearance; slim thickening; cap wall coverage to keep scenario playable."""
    cfg = DIFFICULTIES[diff]; rng = random.Random(cfg.get("seed", None))
    walls, reserved = set(), set()
    total = PLAY_W * PLAY_H
    max_walls = int(total * (1.0 - MIN_PASSABLE_RATIO))

    def can_add(n): return (len(walls) + n) <= max_walls

    # 1) Borders (reserve distance from edges + start)
    ...
    # 2) Random segments with clearance, trimmed if hitting coverage cap
    ...
    while placed<segs and tries>0 and len(walls)<max_walls:
        ...
        # trim if this would exceed wall budget
    ...
    # 3) Slim thickening/connect for a subset of components (uses 4-neighbor rim + sampled growth)
    comps = _comps(walls)
    if comps:
        ...
        for comp in targets:
            ...
            for _ in range(layers):
                ...
                rim=set()
                for x,y in comp:
                    for nx,ny in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)):  # 4-neighbor -> slimmer growth
                ...
                rng.shuffle(elig); add=set(elig[:take])
...
def draw_world(play_batch, walls, victims):
    ...
    def comps(wset): return _comps(wset)
    C = comps(walls); orange=set()
    if C:
        k=int(len(C) * WALL_ORANGE_PCT)
        if k>0:
            for i in random.sample(range(len(C)), k): orange.update(C[i])
    ...
    victim_shapes={p:(shapes.Circle(p[0]*CELL_SIZE+CELL_SIZE/2,
                                    ...
                                    CELL_SIZE/2-3, color=victim_color(t),
                                    batch=play_batch), t) for p,t in victims.items()}
